...
def get_best_index_list(input_str):
    ...
    len_possible_index_list = len(possible_index_list)
    for index in range(len_possible_index_list):
        tmp_cosine_sim.append(
            [float(cosine_similarity(train_v[possible_index_list[index]], test_array)), possible_index_list[index]])
...
# %%
# get_qalist_with_list("我", 1, True)

def filter_similarity(input_tmp, degree=0.6):
    tmp_return = []
    for v in input_tmp:
        if v[0] >= degree:
            tmp_return.append(v)
...
def get_retrieval_ans(input_str, range_int=1):
    tmp_return = get_best_index_list(input_str)
    tmp_return = filter_similarity(tmp_return, 0.8)
    result_list_len = len(tmp_return)
    ans = []
    if result_list_len != 0:
        range_int = result_list_len if result_list_len < range_int else range_int
        for i in range(range_int):
            ans.append(a_list[tmp_return[i][1]])
    return ans, len(ans)
